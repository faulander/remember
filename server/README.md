...
Minimaler modularer Go-Monolith für Remember. Der aktuelle Stand enthält Konfiguration, SQLite-Migrationen, Betriebsprobes, Identity-, Sessions- und Sync-Cores sowie einen mandantengebundenen, auditierbaren SHA-256-Blob-Speicher. Öffentlich verfügbar sind begrenzte Registrierungs-, Auth-, Blob- und Sync-Transporte; Recovery bleibt intern.
...
| `REMEMBER_SMTP_ADDR` | leer; implizite-TLS-Adresse wie `smtp.example.com:465` |
| `REMEMBER_SMTP_USERNAME` | leer |
| `REMEMBER_SMTP_PASSWORD` | leer |
| `REMEMBER_SMTP_FROM` | leer; reine Mailbox wie `remember@example.com` |
| `REMEMBER_SMTP_TIMEOUT` | `10s` |
| `REMEMBER_EMAIL_TOKEN_KEY` | leer; kanonisches ungepolstertes Base64url eines zufälligen 32-Byte-Schlüssels |
...
Registrierung ist nur aktiv, wenn SMTP-Adresse, Benutzername, Passwort, Absender und `REMEMBER_EMAIL_TOKEN_KEY` gemeinsam gesetzt sind. Der SMTP-Adapter verlangt implizites TLS ab Version 1.2 und prüft den Servernamen; es gibt keinen Klartext- oder opportunistischen STARTTLS-Fallback. Der Token-Schlüssel muss unabhängig vom SMTP-Passwort erzeugt, wie ein Servergeheimnis gesichert und bei Neustarts unverändert bereitgestellt werden. Noch nicht zugestellte Tokens sind damit benutzergebunden per AES-256-GCM versiegelt; die Verifikationstabelle enthält ausschließlich den domain-separierten Hash.
...
- `POST /v1/auth/register` (nur bei konfigurierter E-Mail-Zustellung)
...
JSON-Requests sind auf 16 KiB begrenzt und werden strikt validiert. Registrierung, E-Mail-Verifikation, Login und Refresh besitzen feste globale und geheimnisgebundene Mindest-Rate-Limits; `429` liefert `Retry-After`. Registrierungsantworten unterscheiden bestehende Adressen nicht von neu angelegten Konten. Der Limiter speichert nur SHA-256-Schlüssel und wertet weder `X-Forwarded-For` noch andere Client-IP-Header aus. Recovery, Batch-Sync, Streaming und Server Push sind nicht enthalten.
...
- Ausgehende Verifikations-E-Mails verwenden ausschließlich implizites TLS; der Server benötigt dafür Netzwerkzugriff auf den konfigurierten SMTP-Endpunkt.
