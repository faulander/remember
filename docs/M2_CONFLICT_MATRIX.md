...
| Move | divergenter konkurrierender Move / `base_revision_mismatch` | **Politik festgelegt; Implementierung teilweise offen** | ADR [0064](adr/0064-m2-divergent-folder-move-policy.md): Der Serverpfad behält die ursprüngliche Folder-UUID, der lokale Verlierer-Subtree wird unter `_Konflikte/Wiederhergestellt` mit neuem Recovery-Root bewahrt. Zunächst wird nur der exakt inode-gebundene leere Root-Folder automatisiert; weitere Formen bleiben bis eigener Manifest-/DAG-Abnahme fail-closed. |
...
3. Die in ADR 0064 freigegebene Divergent-Move-Politik zunächst für leere root-level Folder mit inode-gebundenem Journal implementieren und danach nur auf exakt manifestierte direkte Note-Ketten erweitern.
