...
| Move | divergenter konkurrierender Move / `base_revision_mismatch`; beide Ziele root-level, lokaler Folder exakt leer | **Konvergiert** | ADR [0064](adr/0064-m2-divergent-folder-move-policy.md)/[0065](adr/0065-m2-empty-divergent-root-folder-move-recovery.md); der Serverpfad behält die ursprüngliche UUID, der exakte lokale Inode erhält unter `_Konflikte/Wiederhergestellt` eine neue UUID. `TestSyncOnceRecoversEmptyDivergentRootFolderMoves`, Fault-Boundary-Tests und ADR [0066](adr/0066-m2-authenticated-empty-divergent-folder-moves.md) mit A/B/Restart/Cold-C über echte HTTP-Routen. Nichtleere, nested und different-parent Formen bleiben fail-closed. |
...
3. Die in ADR 0065 implementierte leere root-level Divergent-Move-Recovery nur nach eigener Manifest-/DAG-Abnahme auf exakt manifestierte direkte Note-Ketten erweitern.
