...
Schema v25 stellt zusätzlich eine dauerhafte Sync-Inbox bereit. Sie trennt `downloaded_cursor` vom bestehenden `confirmed_cursor`, nimmt ausschließlich vollständige lückenlose Pull-Seiten atomar auf, erlaubt nur payload-identischen Replay und hält pro Zeile den monotonen Zustand `pending → applying → applied`. `LocalCore.SyncOnce` spiegelt den weiterhin aktiven Legacy-Pfad in diese Inbox: validierte Seiten werden vor dem Apply-Plan ingestiert, vorhandene Pläne beim Neustart payload-identisch aufgenommen und erfolgreich bestätigte Cursor anschließend transaktional in den Inbox-Zustand übertragen. Ein Fehler nach Ingest lässt den Download-Cursor vorauslaufen; der nächste Pull startet unverändert am Confirmed-Cursor und muss dieselbe Seite exakt wiederholen.

Schema v26 bindet eine pending Root-Note-Update/-Delete-Zeile unveränderlich an einen Einzel-Apply-Plan. Er verlangt die exakte Objekt-Baseline der Vorgängerrevision und keine frühere pending/applying Zeile desselben Objekts. Der Abschluss darf die Objekt-Baseline unabhängig von früheren Cursorzeilen fortschreiben, verschiebt `confirmed_cursor` aber ausschließlich über den zusammenhängenden applied-Präfix. ADR 0057 aktiviert darauf einen begrenzten Scheduler nur bei weiterhin ungelöstem Outbox-Zustand: Pull-Seiten werden ab `downloaded_cursor` ingestiert; danach laufen höchstens 32 unabhängige Root-Note-Updates/-Deletes durch den bestehenden Apply-Pfad. Der Scheduler prüft zusätzlich den fehlenden lokalen Intent, lädt nach jedem Fortschritt Kandidaten neu und zieht dadurch lineare Folgerevisionen desselben Objekts erst nach erfolgreicher exakter Vorgänger-Baseline nach, ruft keinen automatischen Abandon auf und gibt nach der Arbeit weiter `ErrUnresolvedOutbound` zurück. Folder, Nested Notes, Creates und Moves bleiben ausgeschlossen; vollständige Objektisolation und eine UI-/Operator-Steuerung bleiben offen. Schema v27 erlaubt den expliziten Retry desselben unveränderlichen, vor Apply abgebrochenen Planlinks nur bei weiterhin exakter Vorgänger-Baseline, pending Inbox/Step, fehlenden Side-Journalen und ohne früheren offenen Objektzustand. Schema v28 zentralisiert wirksame ungelöste lokale Objekt-Intents in einer SQL-Sicht, filtert sie vor der Kandidatenbegrenzung und prüft dieselbe Bedingung atomar bei Planerzeugung und Retry; damit können 1000 blockierte Objekte keine spätere unabhängige Root-Note verhungern lassen. ADR 0064 legt für divergente Folder Move/Move fest: Der kanonische Serverpfad behält die ursprüngliche Folder-UUID; der lokale Verlierer wird mit neuem Recovery-Root unter `_Konflikte/Wiederhergestellt` bewahrt. Schema v30 setzt den ersten technischen Schnitt für leere root-level Folder mit exakter Device-/Inode-Bindung um. Ein monotones Journal persistiert die kanonische Stage-Identität vor Publish, kompensiert jeden Fehler vor `evacuated` zurück zum lokalen Originalpfad und schließt erst gegen eine exakt angewendete immutable Inbox-Move-Zeile ab. ADR 0066 nimmt diesen Schnitt über echte A/B-Logins, B-Restart und kalten C-Bootstrap mit exakten Original-/Recovery-UUIDs und erhaltenem Verlierer-Inode ab. Schema v31 erweitert ausschließlich auf direkte lokal erstellte Notes mit vollständigen linearen, nie versuchten Create→Update-Ketten. Immutable Member-/Chain-Manifeste, descriptor-retained exakte Subtree-Prüfung und atomare Replacement-Creates erhalten Note-UUIDs und finale Bytes; Branches, attempted, Nested und unbekannte Inhalte bleiben fail-closed. ADR 0068 nimmt den direkten Note-Ketten-Scope über echte A/B-Logins, B-Restart und kalten C-Bootstrap mit exakten Bytes, Note-/Folder-UUIDs und erhaltenem Recovery-Inode ab. ADR 0069 legt für Folder Delete gegen Remote-Move eine atomare servergestützte Preserve-and-Delete-Operation fest: Der Server bindet die unveränderliche Konfliktrevision, klont den vollständigen aktuell aktiven Subtree unter neuen UUIDs parent-first nach `_Konflikte/Wiederhergestellt` und tombstoned das Original child-first in derselben Transaktion. ADR 0070 implementiert den actor-gebundenen idempotenten Server-Core zunächst für aktuell leere Folder; ADR 0071 exponiert diesen engen Core als strikten actor-gebundenen HTTP-Transport; ADR 0072 ergänzt Client-Schema v32 mit unveränderlicher konfliktgebundener Resolution und den strikten HTTP-Client. ADR 0073 integriert automatische `SyncOnce`-Resolution und normalen Change-Log-Apply für aktuell leere Folder und nimmt A/B/Cold-C authentifiziert ab. Rekursives Clone/Tombstone bleibt noch zu implementieren. Schema v29 persistiert fehlende beziehungsweise hashinkonsistente Apply-Blobs plan-, cursor-, objekt- und hashgebunden als deduplizierte offene Integritätsalarme. Der Confirmed-Cursor und das pending Apply bleiben dabei unverändert; `LocalCore` und Desktop-State machen die Alarme sichtbar. ADR 0063 nimmt Missing- und typisierte Hash-Mismatch-Antworten über echte Login-, Sync- und Blob-Routen ab, bewahrt und dedupliziert den Alarm über Restart und setzt denselben Plan nach Wiederherstellung fort. ADR 0058 nimmt diesen engen Scope über echte Auth-/Blob-/Sync-Routen mit A/B und kaltem C ab: Ein divergenter Folder-Move bleibt als ungelöster Intent fail-closed, während unabhängige Y-/Z-Änderungen out-of-order angewendet und ein unabhängiger lokaler Q-Create weiterhin übermittelt werden.

Schema v39 erweitert den begrenzten Scheduler auf Note-Updates/-Deletes in bis zu 256 Folder-Ebenen. Jeder Apply-Plan bindet die vollständige Ancestor-Kette unveränderlich an Tiefe, ID, Parent, relativen Pfad, Device/Inode und exakte Baseline. Kandidatensicht, Plananlage, Resume, Dateisystemschritt und Abschluss schließen fehlende, ersetzte, verschobene, unsynchronisierte oder intent-belegte Ancestors aus. Creates, Moves, Folder-Mutationen und unvollständig beweisbare Ketten bleiben pending; Downloaded-/Confirmed-Cursor und der spätere lückenlose Legacy-Abschluss bleiben unverändert.

ADR 0076 und Client-Schema v38 erweitern lokale Folder-Create-, divergente Root-Folder-Move- und Folder-Move/Remote-Delete-Recovery auf exakt manifestierte Nested-Folder-DAGs. Zulässig sind ausschließlich nie versuchte pending Folder-/Note-Creates mit linearen Note-Updates, genau einer Parent-Dependency und ohne Baseline oder externe Historie. Nur der Verlierer-Root erhält eine neue Folder-UUID; Descendant-UUIDs bleiben erhalten. Relative Pfade, Folder-Device/Inode, Note-Hashes und sämtliche alten/neuen Operations-IDs werden versiegelt, vor der Umschreibung descriptor-relativ verifiziert und parent-first ersetzt.
...
ADR 0080 und Client-Schema v41 öffnen dieselbe Recovery für Root-, gleiche verschachtelte und unterschiedliche verschachtelte Ziel-Parents. Beide Zielseiten persistieren den vollständigen relativen Pfad und versiegeln jede nichtleere Ancestor-Kette mit Seite, Tiefe, Objekt-/Parent-ID, Pfad, exakter Baseline-Revision/-Operation und Folder-Device/Inode. Planung, Resume, Evakuierung, kanonische Publication und Completion prüfen diese Bindungen erneut; fehlende, bewegte, ersetzte, intent-belegte oder revisionsveränderte Parents bleiben fail-closed. Der authentifizierte Konfliktsnapshot liefert bereits die kanonische Parent-ID, daher bleibt das Wire-Protokoll unverändert.
...
## 14.2.2.1 Folder-Create-Pfadkollision
...
- Ein streng manifestierter lokaler Unterbaum aus nie versuchten Folder-/Note-Creates mit linearen Note-Updates wird transaktional auf eine neue Root-UUID umgebunden; Descendant-UUIDs und finale Note-Bytes bleiben erhalten. `VerifyRootedSubtreeExpected` bestätigt vorher den vollständigen manifestierten Baum über descriptor-relative `openat(..., O_NOFOLLOW)`-Traversal, exakte Namen/Typen, Folder-Device/Inode und Datei-SHA-256.
- Move/Delete-, verzweigte, versuchte, externe oder baseline-gebundene Descendant-Historien sowie unerwartete Einträge bleiben fail-closed.
...
- Für rekursive Verlierer gilt dasselbe versiegelte Manifest wie bei der Pfadkollision; Move/Delete-, verzweigte, externe oder versuchte Descendant-Historien bleiben fail-closed.

## 14.2.2.3 Authentifizierte Strukturkonflikt-Abnahme

- Drei unabhängige Geräte durchlaufen über echte Login-, Blob- und Sync-HTTP-Routen beide Note-Move/Delete-Reihenfolgen, lokale Folder-Create-Recovery und Folder Delete gegen Remote-Move.
- Ein kaltes Drittgerät muss ausschließlich aus Cursor-Historie dieselben Konfliktkopien, Tombstones, Recovery-Pfade, Folder-Mappings, Note-UUIDs und Bytes herstellen.
- Die rekursive Abnahme ergänzt Nested-Folder-Create mit linearen Note-Updates, einen gemischten dreistufigen Preserve-delete-Subtree sowie tief verschachtelte Note-Updates/-Deletes hinter einem unbeteiligten ungelösten Konflikt. Restart, Pagination, verlorene Antwort, Original-Tombstone, Recovery-UUIDs, Note-UUIDs und exakte finale Bytes sind explizite Invarianten.
...
- Derselbe Tombstone-Schnitt erlaubt einen exakt manifestierten lokalen Nested-Folder-DAG aus nie versuchten Folder-/Note-Creates mit linearen Note-Updates: Der Root-Inode wird unter neuer UUID verschoben, Descendant-UUIDs und finale Bytes bleiben erhalten, alte Ketten werden atomar parent-first ersetzt.
- In der umgekehrten Richtung löst Protokoll v4 einen lokalen Folder-Delete gegen Remote-Move atomar auf: Alle aktiven Folder werden unter neuen UUIDs geklont, Notes mit unveränderter UUID und exaktem Blob in die jeweiligen Clone-Parents verschoben und Original-Folder deepest-first tombstoned.
- Unerwartete lokale Einträge, nichtlineare oder versuchte lokale Historie, Post-Frontier-Serverhistorie, mehr als 256 Ebenen oder 10.000 Objekte bleiben fail-closed.
...
- ADR 0079 konvergiert gemischte Subtrees aus exakt baseline-identischen serverbekannten und streng manifestierten lokalen Descendants; ADR 0080 erweitert diesen Schnitt auf Root-, gleiche verschachtelte und unterschiedliche verschachtelte Ziel-Parents mit vollständig versiegelter Ancestry. Der Serverpfad behält alle ursprünglichen UUIDs und Bytes; die lokale Verlustfassung klont serverbekannte Identitäten und erhält lokale Identitäten. Canonical-Staging, Recovery-Evakuierung und Replacement-Outbox sind crash-fortsetzbar.
