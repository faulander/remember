...
Schema v36 erweitert diesen Scheduler ausschließlich auf Update-/Delete-Ketten direkter Notes unter einem synchronisierten Root-Folder. Die gemeinsame SQL-Kandidatensicht und die Link-/Retry-Trigger verlangen zusätzlich eine bekannte Root-Parent-ID, gespeicherte Device-/Inode-Identität, eine synchronisierte Parent-Baseline sowie weder frühere offene Inbox-Zeilen noch wirksame lokale Intents für Parent oder Note. Parent-Moves, unbekannte Identitäten, tiefere Ancestry und andere Objektformen bleiben fail-closed. Die authentifizierte A/B/Restart/Cold-C-Abnahme erzwingt kleine Pull-Seiten und belegt exakte Bytes/UUIDs ohne zweite Publikation.

ADR 0075 erweitert die atomare Folder-Delete/Remote-Move-Resolution auf direkte Notes: Folder werden in die Recovery-DAG geklont, Notes dagegen unter unveränderter UUID und mit exakt demselben Blob in den Recovery-Root verschoben. Client und Server versiegeln Recovery-Root-Name, Counts, Ordinals, Cursor sowie Folder- und Note-Deskriptoren; nur exakt mappinggebundene lokale Child-Delete-Operationen werden superseded. Ein vorbereiteter v2-Versuch darf erst nach authentifizierter expliziter `preserve_delete_unavailable`-Antwort dauerhaft mit neuer Resolution-Operations-ID auf v3 wechseln; mehrdeutige Fehler bleiben replay-sicher unverändert. Direkte Folder-Children müssen weiterhin leer sein, rekursive Subtrees bleiben fail-closed.
...
- Die nachfolgenden Zellen werden zusätzlich gemeinsam über authentifizierte Drei-Geräte-Pfade abgenommen: leerer Folder-Move gegen Remote-Delete, äquivalente Root-/Nested-Note-Moves, divergente Same-Parent-Note-Moves sowie direkte Create→Update-Note-Subtrees bei Folder-Create-Kollision und Folder-Move/Remote-Delete. Dabei bleiben Original-Tombstone, Recovery-UUIDs, Note-UUIDs und exakte finale Bytes explizite Invarianten.
