...
Schema v26 bindet eine pending Root-Note-Update/-Delete-Zeile unveränderlich an einen Einzel-Apply-Plan. Er verlangt die exakte Objekt-Baseline der Vorgängerrevision und keine frühere pending/applying Zeile desselben Objekts. Der Abschluss darf die Objekt-Baseline unabhängig von früheren Cursorzeilen fortschreiben, verschiebt `confirmed_cursor` aber ausschließlich über den zusammenhängenden applied-Präfix. ADR 0057 aktiviert darauf einen begrenzten Scheduler nur bei weiterhin ungelöstem Outbox-Zustand: Pull-Seiten werden ab `downloaded_cursor` ingestiert; danach laufen höchstens 32 unabhängige Root-Note-Updates/-Deletes aus einem einmaligen Kandidatensnapshot durch den bestehenden Apply-Pfad. Der Scheduler prüft zusätzlich den fehlenden lokalen Intent, lädt nach jedem Fortschritt Kandidaten neu und zieht dadurch lineare Folgerevisionen desselben Objekts erst nach erfolgreicher exakter Vorgänger-Baseline nach, ruft keinen automatischen Abandon auf und gibt nach der Arbeit weiter `ErrUnresolvedOutbound` zurück. Folder, Nested Notes, Creates und Moves bleiben ausgeschlossen; vollständige Objektisolation und eine UI-/Operator-Steuerung bleiben offen. Schema v27 erlaubt den expliziten Retry desselben unveränderlichen, vor Apply abgebrochenen Planlinks nur bei weiterhin exakter Vorgänger-Baseline, pending Inbox/Step, fehlenden Side-Journalen und ohne früheren offenen Objektzustand. ADR 0058 nimmt diesen engen Scope über echte Auth-/Blob-/Sync-Routen mit A/B und kaltem C ab: Ein divergenter Folder-Move bleibt als ungelöster Intent fail-closed, während unabhängige Y-/Z-Änderungen out-of-order angewendet und ein unabhängiger lokaler Q-Create weiterhin übermittelt werden.
