ALTER TABLE sync_conflict_resolutions RENAME TO sync_conflict_resolutions_v20;
...
INSERT INTO sync_conflict_resolutions SELECT * FROM sync_conflict_resolutions_v20;
DROP TABLE sync_conflict_resolutions_v20;
...
WHEN NEW.resolution='note_move_equivalent' AND NOT EXISTS(
 ...
 AND o.parent_id IS NULL AND c.object_type='note' AND c.deleted=0 AND c.parent_id IS NULL AND c.name=o.name AND c.revision>o.base_revision AND length(c.blob_hash)=32
) BEGIN SELECT RAISE(ABORT,'equivalent note move resolution requires exact root target'); END;
