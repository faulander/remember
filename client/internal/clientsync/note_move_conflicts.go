...
// ResolveEquivalentRootNoteMove records that an already-canonical root target
// satisfies a stale move without changing local bytes or paths.
func (s *Store) ResolveEquivalentRootNoteMove(ctx context.Context, operationID uuid.UUID) error {
...
AND o.parent_id IS NULL AND c.object_type='note' AND c.deleted=0 AND c.parent_id IS NULL AND c.name=o.name AND c.revision>o.base_revision AND length(c.blob_hash)=32`, s.clock().UTC().UnixMilli(), operationID.String())
		if err != nil {
			return err
		}
