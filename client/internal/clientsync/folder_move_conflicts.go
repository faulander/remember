...
func validFolderMoveRevert(r ConflictFolderMoveRevert) bool {
	return validOperationID(r.OperationID) && validObjectID(r.FolderID) && naming.ValidateUserRelativePath(r.AttemptedRelative) == nil && naming.ValidateUserRelativePath(r.CanonicalRelative) == nil && r.AttemptedRelative != r.CanonicalRelative && r.Device > 0 && r.Inode > 0 && r.Device <= math.MaxInt64 && r.Inode <= math.MaxInt64 && (r.State == "prepared" || r.State == "moved" || r.State == "completed")
}
