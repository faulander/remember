...
- ADR 0080 und Client-Schema v41 erweitern die gemischte divergente Folder-Move-Recovery auf Root-, gleiche verschachtelte und unterschiedliche verschachtelte Ziel-Parents. Beide vollständigen Ancestor-Ketten sind mit Parent-ID, Pfad, Baseline-Operation/-Revision und Device/Inode versiegelt und werden vor Resume, Publication und Completion erneut geprüft. Migrationserhalt, Parent-Tampering, kanonische/recoverte Identitäten sowie authentifizierte A/B/Restart/Cold-C-Konvergenz sind getestet; das Wire-Protokoll bleibt unverändert.
- ADR 0079 und Client-Schema v40 konvergieren divergente Root-Folder-Moves mit gemischten serverbekannten und lokalen Descendants. Der kanonische Subtree behält alle bestätigten UUIDs und Bytes; die lokale Verlustfassung klont serverbekannte Folder/Notes unter frischen UUIDs und erhält lokale UUIDs sowie finale Bytes. Versiegeltes Parent-/Baseline-/Inode-/Hash-/Operations-Manifest, Fault-Boundary-Resume und authentifizierte A/B/Restart/Cold-C-Konvergenz sind getestet.
- ADR 0078 und Client-Schema v39 erweitern den Out-of-order-Scheduler auf Note-Updates/-Deletes in bis zu 256 Ebenen tiefen Foldern. Jeder Ancestor ist mit ID, Parent, Pfad, Device/Inode und exakter Baseline gebunden und wird vor Planung, Resume, Dateisystemmutation und Abschluss erneut geprüft; A/B/Restart/Cold-C, Pagination und Parent-Austausch sind abgenommen.
- ADR 0077 und Protokoll v4 implementieren die vollständige rekursive ADR-0069-DAG: Der Server klont bis zu 10.000 aktive Folder-/Note-Objekte bei höchstens 256 Folder-Ebenen atomar, verschiebt Notes identitätserhaltend in die geklonten Parents und tombstoned Original-Folder deepest-first. Servermigration 010, Client-Schema v37, 32-MiB-Transportgrenze, exakter V1–V3-Replay und crash-fortsetzbarer A/B/Restart/Cold-C-Apply sind getestet.
- ADR 0076 und Client-Schema v38 retten vollständig manifestierte lokale Nested-Folder-DAGs bei Folder-Create-Kollision beziehungsweise gelöschtem Parent, divergentem Root-Folder-Move und Folder-Move/Remote-Delete. Zulässig bleiben ausschließlich nie versuchte Folder-/Note-Creates mit linearen Note-Updates; Manifest, Source-Identitäten, Ersatzoperationen und parent-first Apply sind versiegelt.
- Client-Schema v36 erweitert den begrenzten Out-of-order-Scheduler auf lineare Update-/Delete-Ketten direkter Notes unter einem unverändert synchronisierten Root-Folder. Kandidaten, Planerzeugung und Retry verlangen die exakte Note-Vorgängerbaseline, eine bekannte Root-Parent-ID mit gebundener Device-/Inode-Identität sowie fehlende lokale oder frühere offene Parent-Intents; Pagination, Restart, unveränderter Confirmed-Präfix und kalter C-Bootstrap sind über echte Auth-/Blob-/Sync-Routen abgenommen.
